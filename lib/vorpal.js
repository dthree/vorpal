...
vorpal._exec = function (item) {
  ...
  function callback(cmd, err, msg, argus, cancelled) {
    ...
    if (cmd.callback) {
      ...
      cmd.reject(msg);
    } else if (cancelled && cmd.onCancel) {
      cmd.onCancel();
    }
  ...
  if (match) {
    ...
    item.session.execCommandSet(item, function (wrapper, err, data, argus, cancelled) {
      callback(wrapper, err, data, argus, cancelled);
    });
  } else {
