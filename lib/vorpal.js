...
var _ = require("lodash")
  ...
  , Command = require("./command")
    , CommandInstance = require("./command-instance")
...
vorpal._exec = function(item) {
  ...
  function callback(cmd, err, msg) {
    if (!err) {
      if (!cmd.callback) {
        ...
        cmd.callback.call(self, undefined, msg);
      }
    } else {
      if (cmd.callback !== undefined) {
        cmd.callback(err, msg);
      } else if (cmd.reject !== undefined) {
        cmd.reject(msg);
      }
    }
  }
  ...
  if (match) {
    ...
    args = self.util.buildCommandArgs(args, match, item);

    // If we get a string back, it's a validation error.
    // Show help and return.
    if (_.isString(args) || !_.isObject(args)) {
      throwHelp(item, args);
      callback(item, undefined, args);
      return;
    }
    ...
    // child command.
    for (var j = item.pipes.length - 1; j > -1; --j) {
      var parent = item.pipes[j - 1];
      var downstream = item.pipes[j + 1];
      item.pipes[j].parent = parent;
      item.pipes[j].downstream = downstream;
    }

    // Create the command instance for the first 
    // command and hook it up to the pipe chain.
    var commandInstance = new CommandInstance({
      downstream: item.pipes[0],
      commandWrapper: item
    });
    ...
    function completionHandler(err, data, argus) {

    }

    // Call the vorpal API function.
    var res = fn.call(commandInstance, args, function() {
      var fixedArgs = self.util.fixArgsForApply(arguments);
      completionHandler()
