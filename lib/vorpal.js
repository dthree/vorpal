...
Vorpal.prototype.parse = function (argv, options) {
  ...
  var args = argv;
  var result = this;
  var catchExists = !(_.findWhere(this.commands, {_catch: true}) === undefined);
  args.shift();
  args.shift();
...
vorpal._exec = function (item) {
  ...
  if (match && _.isFunction(match._parse)) {
    item.command = match._parse(item.command, matchParts.args);
  }

  // Grab additional pipes from the parse.
  var parsedPipes = String(item.command).trim().split('|').map(function (itm) {
    return String(itm).trim();
  });
  parsedPipes.shift();
  item.pipes = item.pipes.concat(parsedPipes);

  matchParts = this.util.matchCommand(item.command, this.commands);
...
*/
  ...
  let match = commandData.match;
  let matchArgs = commandData.matchArgs;
