...
import buildCommandArgs from './utils/buildCommandArgs';
...
import prettifyArray from './utils/prettifyArray';
...
vorpal._exec = function (item) {
  ...
  function throwHelp(cmd, msg, alternativeMatch) {
    ...
    cmd.session.log(pickedMatch.renderHelp());
  }
  ...
  if (match) {
    ...
    for (var j = 0; j < item.pipes.length; ++j) {
      ...
      try {
        ...
        commandParts.args = error.message;
