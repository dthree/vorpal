...
const buildCommandArgs = require('./utils/buildCommandArgs');
const humanReadableArgName = require('./utils/humanReadableArgName');
const matchCommand = require('./utils/matchCommand');
const pad = require('./utils/pad');
const padRow = require('./utils/padRow');
const parseCommand = require('./utils/parseCommand');
const prettifyArray = require('./utils/prettifyArray');

/**
 * Module dependencies. (OLD)
 */
  ...
  if (this.session.isLocal() && !this.session.client && !this._command) {
    this.session.getKeypressResult(key, value, function (err, result) {
      if (!err && result !== undefined) {
        if (_.isArray(result)) {
          var formatted = prettifyArray(result);
          self.ui.imprint();
...
vorpal._exec = function (item) {
  ...
  if (match) {
    ...
    item.args = buildCommandArgs(matchArgs, match, item, self.isCommandArgKeyPairNormalized);
    ...
    for (var j = 0; j < item.pipes.length; ++j) {
      var commandParts = matchCommand(item.pipes[j], self.commands);
      if (!commandParts.command) {
        item.session.log(self._commandHelp(item.pipes[j]));
        allValid = false;
        break;
      }
      commandParts.args = buildCommandArgs(commandParts.args, commandParts.command);
...
vorpal._commandHelp = function (command) {
  ...
  var commands = matches.filter(function (cmd) {
    ...
    var args = cmd._args.map(humanReadableArgName).join(' ');
  ...
  var groups = _.uniq(matches.filter(function (cmd) {
    ...
    let prefix = `    ${pad(cmd + ' *', width)}  ${counts[cmd]} sub-command${((counts[cmd] === 1) ? '' : 's')}.`;
    return prefix;
  });
  ...
  var commandsString = (commands.length < 1) ? '' : '\n  Commands:\n\n' +
    commands.map(function (cmd) {
      var prefix = '    ' + pad(cmd[0], width) + '  ';
      var suffix = wrap(cmd[1], descriptionWidth - 8).split('\n');
      for (let i = 0; i < suffix.length; ++i) {
        if (i !== 0) {
          suffix[i] = pad('', width + 6) + suffix[i];
        }
      }
      suffix = suffix.join('\n');
