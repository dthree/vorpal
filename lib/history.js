...
const DEFAULT_STORAGE_PATH = path.normalize(path.join(os.tmpdir(), '.local_storage'));
...
class History {
  ...
  setStoragePath(newPath) {
    if (!this._localStorage) {
      this._localStorage = new LocalStorage(newPath);
    }
  }
  ...
  newCommand(command) {
    ...
    if (this._hist[this._hist.length - 1] === command) {
      return;
    }

    // Push into history.
    this._hist.push(command);
